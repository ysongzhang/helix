...
/**
 * send to all o[player_no]
 * reveive the other o[i]
 */
void Player::unchecked_broadcast(vector<octetStream>& o) const
{
    // Send to all
    send_all(o[player_no]);
    // receive
    for (int i = 0; i < nplayers; i++){
        if(i != player_no){
            receive_player(i, o[i]);
        }
    }
}
...
void Player::update_hash_state(const octet* dataIn, size_t lenIn) const
{
    hash_update(&ctx,dataIn,lenIn);
}
