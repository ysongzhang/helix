...
CORES=4
MULT_COMPRESSION=4
VERIFY_PROTOCOL=Falcon
NET=LAN

IP_FILE=Inference/IP_HOSTS/IP_LOCAL
BASE_FILE=Inference/$(NETWORK)
Output_file=$(BASE_FILE)/$(NETWORK).P0
OFFLINE_ARG=$(BASE_FILE)/offline/$(PRIME)_offline_b$(TEST_DATA_SIZE)_${VERIFY_PROTOCOL}.txt

terminal: inference.x
	./inference.x 2 $(IP_FILE) $(THRESHOLD) $(NETWORK) $(DATASET) $(TEST_DATA_SIZE) $(TRUE_OFFLINE) $(OFFLINE_ARG) $(CORES) $(MULT_COMPRESSION) $(VERIFY_PROTOCOL) > /dev/null &
	./inference.x 1 $(IP_FILE) $(THRESHOLD) $(NETWORK) $(DATASET) $(TEST_DATA_SIZE) $(TRUE_OFFLINE) $(OFFLINE_ARG) $(CORES) $(MULT_COMPRESSION) $(VERIFY_PROTOCOL) > /dev/null &
	./inference.x 0 $(IP_FILE) $(THRESHOLD) $(NETWORK) $(DATASET) $(TEST_DATA_SIZE) $(TRUE_OFFLINE) $(OFFLINE_ARG) $(CORES) $(MULT_COMPRESSION) $(VERIFY_PROTOCOL)
	@echo "Execution completed"
...
one: inference.x
	./inference.x 1 $(IP_FILE) $(THRESHOLD) $(NETWORK) $(DATASET) $(TEST_DATA_SIZE) $(TRUE_OFFLINE) ${OFFLINE_ARG} ${CORES} $(MULT_COMPRESSION) $(VERIFY_PROTOCOL)
	@echo "Execution completed"

two: inference.x
	./inference.x 2 $(IP_FILE) $(THRESHOLD) $(NETWORK) $(DATASET) $(TEST_DATA_SIZE) $(TRUE_OFFLINE) ${OFFLINE_ARG} ${CORES} $(MULT_COMPRESSION) $(VERIFY_PROTOCOL)
	@echo "Execution completed"
