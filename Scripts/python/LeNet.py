...
root = "/home/helix/Inference/LeNet/"
path = root + "preload/"
np.savetxt(fname=path+"weight1", delimiter=" ", X=params[0][1].reshape(5*5*1, 20).tolist())
np.savetxt(fname=path+"bias1", delimiter=" ", X=params[1][1].tolist())
np.savetxt(fname=path+"weight2", delimiter=" ", X=params[2][1].reshape(5*5*20, 50).tolist())
...
for img, label in test_loader:
    ...
    np.savetxt(fname=path_tmp+"outputlayer1", delimiter=" ", X=model.output(img)[0].data.cpu().view(-1))
