...
BitBundle bitwise_xor(const BitBundle&a, const BitBundle &b)
{
    ...
    // The length of b.cols() may larger that a.cols(), thus use leftCols to obtain the first a.cols() columns of b.shares
    partRes.shares = a.shares.array() * b.shares.array().leftCols(a.cols());
    partRes.reduce_degree();
    
    // Verify
    MultVerifier::push_triples(a.shares, b.shares.leftCols(a.cols()), partRes.shares);
...
BitBundle less_than_unsigned(const BitBundle &a, const BitBundle &b)
{
    ...
    for (size_t i = 0; i < a.rows(); i++)
    {
        MultVerifier::push_dot_triple(deltaXor.shares.row(i)(seqN(0, b.cols())).transpose(), b.shares.row(i).transpose(), res.shares(i));
    }
...
// Time = 2 round
// This code does not use the optimized protocol in paper.
template<typename Derived>
...
ShareBundle BitBundle::if_else(const ShareBundle&a, const ShareBundle &b)
{
    ...
    res.shares = shares.array() * a.shares.array() + (1 - shares.array()) * b.shares.array();  // cond * a + (1 - cond) * b = b + cond * (a - b)
    res.reduce_degree();

    // Verify
    MultVerifier::push_triples(shares, (a.shares - b.shares), (res.shares - b.shares));
...
BitBundle BitBundle::prefix_op_opt(string fn)
{
    BitBundle res(rows(), cols());
    if(fn=="AND"){
        res.shares = prefix_mult_log4_round().shares;
        return res;
    }
    ...
    if(fn=="OR"){
        mapped.shares = 1 - shares.array();
        res.shares = 1 - mapped.prefix_mult_log4_round().shares.array();
        return res;
    }
}

BitBundle BitBundle::postfix_op_opt(string fn)
{
    BitBundle res(rows(), cols());
    if(fn=="AND"){
        res.shares = postfix_mult_log4_round().shares;
        return res;
    }
    ...
    if(fn=="OR"){
        mapped.shares = 1 - shares.array();
        res.shares = 1 - mapped.postfix_mult_log4_round().shares.array();
        return res;
    }
}
