...
PRNG ShareBase::PRNG_agreed;  // shared among n parties
...
gfpMatrix ShareBase::reconstruction_with_n_points_t; // This is to reconstruct the secret and P{t+2-n} shares from the known P{1-t+1} shares.

// reuse factor for reconstructing multiple points
gfpVector ShareBase::reuse_factor_extend_compress;  // This is used in multiplication verification protocol step two.
gfpVector ShareBase::reuse_factor_compress;  // This is used in multiplication verification protocol step three.
...
void ShareBase::init_reconstruction_vectors()
{
    ...
    reconstruction_with_n_points_t.resize(n_players-threshold, threshold+1);
    // First row
    reconstruction_with_n_points_t.row(0) = get_reconstruction_vector_with_n_points(gfpScalar(0), threshold);
    // Next n-t-1 rows
    for (int i = threshold+2; i < n_players+1; i++)
    {
        reconstruction_with_n_points_t.row(i-threshold-1) = get_reconstruction_vector_with_n_points(gfpScalar(i), threshold);
    }
}
...
// Generate reuse factor according to degree
void ShareBase::init_reuse_factor_extend_compress(const int &degree)
{
    int n_points = degree+1;
    gfpVector reconstruction(n_points);
    for(int player_i = 0; player_i < n_points; player_i++){
        gfpScalar factor(1);
        gfpScalar gfp_i(player_i);
        for(int player_j = 0; player_j < n_points; player_j++){
            gfpScalar gfp_j(player_j);
            if(player_i!=player_j){
                factor *= (gfp_j - gfp_i);  // could reuse
            }
        }
        reconstruction(player_i) = factor;
    }
    reuse_factor_extend_compress = reconstruction;
}

void ShareBase::init_reuse_factor_compress(const int &degree)
{
    int n_points = degree+1;
    gfpVector reconstruction(n_points);
    for(int player_i = 0; player_i < n_points; player_i++){
    ...
    reuse_factor_compress = reconstruction;
}
...
// player_i = x_i
gfpScalar ShareBase::get_reconstruction_factor_general(const gfpScalar &point,const int &player_i,const int &degree, const gfpVector &reuse_factor)
{
    gfpScalar factor(1);
    int n_points = degree+1;
    gfpScalar gfp_i(player_i);
    ...
    factor /= reuse_factor(player_i);
    
    return factor;
}

gfpScalar ShareBase::get_reconstruction_factor_general(const gfpScalar &point,const int &player_i,const int &degree)
{
    gfpScalar factor(1);
    int n_points = degree+1;
    gfpScalar gfp_i(player_i);
...
{
    int n_relevant_player = degree+1;
    gfpVector reconstruction(n_relevant_player,1);  // gfpVector reconstruction(n_relevant_player);
...
// Note: point must be larger than degree (point > degree)
gfpVector ShareBase::get_reconstruction_vector_general(const gfpScalar &point, const int &degree, const gfpVector &reuse_factor)
{
    assert(point.get_value() > degree);
    int n_points = degree+1;
    assert(n_points == reuse_factor.size());
    gfpVector reconstruction(n_points);
    for(int i = 0; i < n_points; i++){
        int player_no = i;
        reconstruction(i) = get_reconstruction_factor_general(point, player_no, degree, reuse_factor);
    }
    return reconstruction;
}

gfpVector ShareBase::get_reconstruction_vector_general(const gfpScalar &point, const int &degree)
{
    assert(point.get_value() > degree);
    int n_points = degree+1;
    gfpVector reconstruction(n_points);
    for(int i = 0; i < n_points; i++){
        int player_no = i;
        reconstruction(i) = get_reconstruction_factor_general(point, player_no, degree);
    }
    return reconstruction;
}
...
gfpScalar ShareBase::get_reconstruction_factor_with_n_points(const gfpScalar &point, const int &player_i, const int &degree)
{
    gfpScalar factor(1);
    int n_points = degree+1;
    gfpScalar gfp_i(player_i);
    
    for(int player_j = 1; player_j < n_points+1; player_j++){
        gfpScalar gfp_j(player_j);
        if(player_i!=player_j){
            factor *= (gfp_j - point)/(gfp_j - gfp_i);
        }
...
gfpVector ShareBase::get_reconstruction_vector_with_n_points(const gfpScalar &point, const int &degree)
{
    assert((point.get_value() > degree+1) || (point.get_value() == 0));
    int n_points = degree+1;
    gfpVector reconstruction(n_points);
    for(int i = 0; i < n_points; i++){
        int player_no = i+1;
        reconstruction(i) = get_reconstruction_factor_with_n_points(point, player_no, degree);
    }
    return reconstruction;
}

void ShareBase::compare_view()
{
    P->Check_Broadcast();
}
...
void Share::receive_share_all(octetStreams& os)
{
...
{
    if(degree==threshold){ distribute_t_sharing_PRG(_secret, _share);}
 ...
 * 
 */
void Share::distribute_t_sharing_PRG(const gfpScalar &_secret, gfpScalar &_share)
{
    ...
    random_matrix(shares_prng, PRNG_agreed);  // BUG: Since the PRNG_agreed is shared among n parties, thus it's insecure. The adversary can obtain at most 2t shares!
...
Share& Share::truncate()
{
    ...
    reveal(true);  // malicious security
    secret.truncate();
...
Share& Share::reduce_truncate()
{
    ...
    reveal();  // semi-honest; 2t-sharing
    degree >>=1;
    mult_checked = secret - ConstEncode - r.aux_aux_share;  // t-sharing
    secret.truncate();
...
Share& Share::reduce_truncate(size_t precision)
{
    ...
    reveal(); // semi-honest; 2t-sharing
    mult_checked = secret - r.aux_aux_share;
    degree >> 1;
...
void Share::input_from_file(int player_no, bool active)
{
    ...
    if (active && (degree==threshold)) {
        // Correctness check
        InputVerifier::push_scalar(share);
    }
}

void Share::input_from_party(int player_no, bool active)
{
...
void Share::input_from_pking(bool active)
{
    input_from_party(Pking, active);
}

void Share::input_from_random(int player_no, bool active)
{
    if(player_no == P->my_num()) {
        secret.random();
        distribute_sharing();
...
void Share::input_from_party_PRG(int player_no, bool active)
{
...
void Share::input_from_random_PRG(int player_no, bool active)
{
    if(player_no == P->my_num()){
...
gfpScalar Share::reveal(bool active)
{
    ...
    if(active && (degree == threshold)){  // Only t-sharings need to be checked!
        RevealVerifier::push_scalar(secret, share);
    }

    return secret;
}

// Only used for t-sharings
void Share::reveal_to_party_with_check(int player_no) 
{
    ...
    if(P->my_num()==player_no){
        // reconstruct_secret()
        octetStreams os(n_players);
        receive_share_all(os);

        // calculate_secret()
        gfpVector sharing(n_players);
        unpack_row(sharing, os);
        sharing(P->my_num())=share;  // n point, from x = 1 to n.
        // Determine whether n points are within a polynomial
        gfpVector reconstruct_points = sharing.block(0, 0, degree+1, 1);
        gfpVector checked_points = sharing.block(degree+1, 0, n_players-degree-1, 1);
        gfpVector computed_points = reconstruction_with_n_points_t * reconstruct_points;
        secret = computed_points(0);
        if (checked_points != computed_points.block(1, 0, n_players-degree-1, 1)) {
            throw reveal_error();
            // cout<<"Share ### reveal with check error: "<<endl;
            // assert(checked_points == computed_points.block(1, 0, n_players-degree-1, 1));
        }
    }
}
...
void DoubleShare::input_from_random_PRG(int player_no)
{
    ...
    if(player_no == P->my_num()){
        secret.random();
        distribute_t_sharing_PRG(secret, share);
...
DoubleShare& DoubleShare::truncated_random(Share &r_msb)
{
    ...
    DoubleRandom::get_random_triple(DoubleRandom::queueTruncatedRandom, share, aux_share, r_msb.share); // [r/2^d]_t, [r]_t, [r_msb]_t
    return *this;
}
...
DoubleShare& DoubleShare::reduced_truncated_random(Share &r_msb)
{
    ...
    DoubleRandom::get_random_quadruple(DoubleRandom::queueReducedTruncatedRandom, share, aux_share, aux_aux_share, r_msb.share); // [r/2^d]_t, [r]_2t, [r]_t, [r_msb]_t
    return *this;
}
...
DoubleShare& DoubleShare::reduced_truncated_random(size_t precision)
{
    ...
    DoubleRandom::get_random_triple(DoubleRandom::queueReducedTruncatedWithPrecisionRandom, share, aux_share, aux_aux_share); // [r/2^d]_2t, [r]_2t, [r]_t
    return *this;
}
