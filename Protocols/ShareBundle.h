...
protected:
    ...
    void send_shares_all();
    void receive_shares_all(octetStreams& os);
    void send_secrets();// send to all other parties.
    void send_secrets(octetStream& o);// send to all other parties.
    void receive_secrets(int player_no);// recv from
    void receive_secrets(int player_no, octetStream& o);
...
public:
    gfpMatrix shares;
    gfpMatrix mult_checked;
    ShareBundle():degree(threshold){}
    // BUG LOG: Need to setConsts(0). resize() dose not initalize to 0.
    ShareBundle(const size_t &len):ShareBundle(){resize(len,1); initializeZero();}  // vector with length = len
    ShareBundle(const size_t &xSize, const size_t &ySize):ShareBundle(){resize(xSize, ySize); initializeZero();}

    void initializeZero(){secrets.setConstant(0); shares.setConstant(0); mult_checked.setConstant(0);}
    void resize(const size_t &xSize, const size_t &ySize)
    {secrets.resize(xSize, ySize); shares.resize(xSize, ySize); mult_checked.resize(xSize, ySize);}
    ...
    void set_share(const size_t &idx, const gfpScalar &_share){shares(idx) = _share;}
    void set_share(const gfpScalar &_share){shares.setConstant(_share);}
    void set_shares(const gfpMatrix &_shares){shares = _shares;}

    // Set in row-grained
    void set_row(const size_t &row_no, const ShareBundle&other)
    {shares.row(row_no)<<other.shares; mult_checked.row(row_no)<<other.mult_checked;}
    ...
    // This input version (request) must be used with finish methods.
    ...
    void finish_input_from(int player_no, octetStreams &os_send, octetStream &o_receive, bool active = false);
    ...
    void finish_input_from(int player_no, bool active = false);
    void finish_input_from_pking(bool active = false);
    ...
    int n_party_PRG()const{return ShareBase::n_players - ShareBase::threshold;}// n-t
    // Normal version
    ...
    void finish_input_from_PRG(int player_no, octetStreams &os_send, const gfpMatrix &shares_prng, octetStream &o_receive, bool active = false);

    // Use the static buffers to input.
    void input_from_party_request_PRG(int player_no);
    void input_from_random_request_PRG(int player_no);
    void finish_input_from_PRG(int player_no, bool active = false);

    // Reveal
    void reveal_to_party(int player_no);  // passive
    void reveal_to_pking();  // passive
    gfpMatrix reveal(bool active = false); // active: defer check
    void reveal_to_party_with_check(int player_no); // active; only for t-sharings.
    gfpMatrix reveal_with_check();  // O(n^2), reveal to all the parties with check; active security; only for t-sharings.
    gfpMatrix reveal_truncate(size_t precision, bool active = false);
    gfpMatrix reveal_truncate(vector<size_t> &precision, bool active = false);
    ...
    void reveal_blocks_dispersed();  // passive
    ...
    void input_blocks_dispersed(bool active = false);
    ...
    void input_blocks_dispersed_PRG(bool active = false);
    ...
    gfpMatrix reveal_dispersed(bool active = false);
    gfpMatrix reveal_truncate_dispersed(size_t precision, bool active = false);// reconstruct the secret and truncate
    gfpMatrix reveal_truncate_dispersed(vector<size_t> &precision, bool active = false);
    ...
    ShareBundle unbounded_mult_3Elems_reused(); // Reuse the first 2 elements and at most compute 4 unbounded mult for each row.
    ShareBundle unbounded_prefix_mult_4Elems();
    ShareBundle unbounded_prefix_mult_3Elems();
    ShareBundle unbounded_postfix_mult_4Elems();
...
public:
    ...
    void resize(const size_t &xSize, const size_t ySize){secrets.resize(xSize, ySize); shares.resize(xSize, ySize); aux_shares.resize(xSize, ySize); aux_aux_shares.resize(xSize, ySize);}

    // Set in row-grained
    void set_row(const size_t &row_no, const DoubleShareBundle&other)
    {shares.row(row_no)<<other.shares; aux_shares.row(row_no)<<other.aux_shares; aux_aux_shares.row(row_no)<<other.aux_aux_shares;}
    ...
    DoubleShareBundle& truncated_random(ShareBundle &msb); // ([r/2^d]_t, [r]_t, [r_msb]_t)
    DoubleShareBundle& truncated_random(size_t precision); // ([r/2^d]_t, [r]_t)
    DoubleShareBundle& reduced_truncated_random(ShareBundle &msb); // ([r/2^d]_t, [r]_2t, [r]_t, [r_msb]_t)
    DoubleShareBundle& reduced_truncated_random(size_t logLearningRate, size_t logMiniBatch);// used in ML ([r/2^d]_t, [r]_2t, [r]_t)
    DoubleShareBundle& reduced_truncated_random(size_t precision); // ([r/2^d]_t, [r]_2t, [r]_t)
    DoubleShareBundle& reduced_truncated_random(vector<size_t>& precision); // ([r/2^d]_t, [r]_2t, [r]_t)

    DoubleShareBundle& unbounded_prefix_mult_random();
};

class DoubleShareBundlePair:public ShareBundle
{
public:
    gfpMatrix aux_shares;
    gfpMatrix other_shares;
    gfpMatrix other_aux_shares;
    gfpMatrix prod_shares;

    DoubleShareBundlePair():ShareBundle(){}
    DoubleShareBundlePair(const size_t &len):DoubleShareBundlePair(){resize(len, 1);}
    DoubleShareBundlePair(const size_t &xSize, const size_t &ySize):DoubleShareBundlePair(){resize(xSize, ySize);}
    void resize(const size_t &xSize, const size_t ySize){
        secrets.resize(xSize, ySize); 
        shares.resize(xSize, ySize); 
        aux_shares.resize(xSize, ySize); 
        other_shares.resize(xSize, ySize);
        other_aux_shares.resize(xSize, ySize);
        prod_shares.resize(xSize, ySize);
    }

    // Set in row-grained
    void set_row(const size_t &row_no, const DoubleShareBundlePair&other){
        shares.row(row_no)<<other.shares; 
        aux_shares.row(row_no)<<other.aux_shares; 
        other_shares.row(row_no)<<other.other_shares;
        other_aux_shares.row(row_no)<<other.other_aux_shares;
        prod_shares.row(row_no)<<other.prod_shares; 
    }

    // Random
    DoubleShareBundlePair& mult_reduced_random_pair(); // ([r1]_t, [r1]_2t), ([r2]_t, [r2]_2t), [r1r2]_t

};

class ReusedDoubleShareBundlePair:public DoubleShareBundlePair
{
public:
    ReusedDoubleShareBundlePair():DoubleShareBundlePair(){}
    ReusedDoubleShareBundlePair(const size_t &len):ReusedDoubleShareBundlePair(){resize(len, 1);}
    ReusedDoubleShareBundlePair(const size_t &xSize, const size_t &ySize):ReusedDoubleShareBundlePair(){resize(xSize, ySize);}
    void resize(const size_t &xSize, const size_t ySize){
        secrets.resize(xSize, ySize); 
        shares.resize(xSize, ySize); 
        aux_shares.resize(xSize, ySize);
        other_shares.resize(xSize<<2, ySize);  // 4 times
        other_aux_shares.resize(xSize<<2, ySize);
        prod_shares.resize(xSize<<2, ySize);
    }

    // Random
    ReusedDoubleShareBundlePair& reused_mult_reduced_random_pairs(); // ([r]_t, [r]_2t), ([r1]_t, [r1]_2t), ([r2]_t, [r2]_2t), ([r3]_t, [r3]_2t), ([r4]_t, [r4]_2t), [r*r1]_t, [r*r2]_t, [r*r3]_t, [r*r4]_t

};

class ReusedDoubleShareBundlePair16:public DoubleShareBundlePair
{
public:
    ReusedDoubleShareBundlePair16():DoubleShareBundlePair(){}
    ReusedDoubleShareBundlePair16(const size_t &len):ReusedDoubleShareBundlePair16(){resize(len, 1);}
    ReusedDoubleShareBundlePair16(const size_t &xSize, const size_t &ySize):ReusedDoubleShareBundlePair16(){resize(xSize, ySize);}
    void resize(const size_t &xSize, const size_t ySize){
        secrets.resize(xSize, ySize); 
        shares.resize(xSize, ySize); 
        aux_shares.resize(xSize, ySize);
        other_shares.resize(xSize<<4, ySize);  // 16 times
        other_aux_shares.resize(xSize<<4, ySize);
        prod_shares.resize(xSize<<4, ySize);
    }

    // Random
    ReusedDoubleShareBundlePair16& reused_mult_reduced_random_pairs_16();
};

class BeaverBundleTriple:public ShareBase
{
public:
    gfpMatrix left_shares;
    gfpMatrix right_shares;
    gfpMatrix prod_shares;

    BeaverBundleTriple(){}
    BeaverBundleTriple(const size_t &xSize, const size_t &ySize, const size_t &zSize){resize(xSize, ySize, zSize);}
    void resize(const size_t &xSize, const size_t &ySize, const size_t &zSize){
        left_shares.resize(xSize, ySize); 
        right_shares.resize(ySize, zSize); 
        prod_shares.resize(xSize, zSize);
    }

    // Get
    size_t x_size()const{return left_shares.rows();}
    size_t y_size()const{return left_shares.cols();}
    size_t z_size()const{return right_shares.cols();}

    // Random
    BeaverBundleTriple& beaver_triple(bool is_offline = false); // [A]_t, [B]_t, [C]_t

    // Beaver multiplication
    gfpMatrix beaver_mult(const gfpMatrix &input_x, const gfpMatrix &input_y);
