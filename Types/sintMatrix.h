...
public:
    ...
    gfpMatrix& checked(){gfpMatrix& ref = sharings.mult_checked; return ref;}
    const gfpMatrix& checked()const{const gfpMatrix &ref = sharings.mult_checked; return ref;}
...
inline sintMatrix operator*(const sintMatrix&a, const sintMatrix&b)
{
    ...
    MultVerifier::push_matrix_triple(a.share(), b.share(), res.share());
    return res;
}
...
inline void sintMatrix::input_from_file(int player_no)
{
    ...
    sharings.input_from_party(player_no, true);
}
...
{
    sharings.input_from_random(player_no, true);
}
...
{
    sharings.finish_input_from(player_no, true);
}
...
{
    sharings.reveal_to_party_with_check(player_no);
    if(ShareBase::P->my_num()==player_no){
...
{
    return cintMatrix(sharings.reveal(true));
}
...
inline sintMatrix& sintMatrix::mult_cwise(const sintMatrix &other)
{
    ...
    MultVerifier::push_triples(tmp, other.share(), share());
    return *this;
}
