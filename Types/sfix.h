...
inline sfix operator*(const sfix&a, const sfix &b)
{
    ...
    res.reduce_truncate();
    MultVerifier::push_triple(a.share(), b.share(), res.mult_checked);
...
inline sfix mult(const sfix&a, const sfix&b, size_t precision)
{
    ...
    MultVerifier::push_triple(a.share(), b.share(), res.sharing.mult_checked);
    return res;
}
...
{
    sharing.input_from_party(player_no, true);
}
...
{
    sharing.reveal_to_party_with_check(player_no);
    if(ShareBase::P->my_num()==player_no){
...
{
    return cfix(sharing.reveal(true));
}
}
